...
const nhifDeductionRate=[
    ...
    { max:89999, rate: 1.67},
    {max:99999, rate: 1.60},
    {max:200000, rate: 1.70},
];

const nssfRates = 0.06;


let individualBasicSalary = parseFloat(prompt( 'Enter Your Basic Salary'));

let individualBenefits = parseFloat(prompt('Enter Your Benefits'));
...
let payee = calculateDeduction(grossSalary, incomeTaxRates);

//We then need to calculate the NHIF Deductions to get

let nhifDeductions = calculateDeduction(grossSalary,nhifDeductionRate);
...
let netSalary = Math.abs(grossSalary - payee - nhifDeductions - nssfDeductions);
...
function calculateDeduction(amount, rateArray) {
    for (let i = 0; i < rateArray.length; i++) {
        if (amount <= rateArray[i].max) {
            return amount * rateArray[i].rate;
        }
    }
    return 0;
}
